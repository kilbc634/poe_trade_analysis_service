...
2. `POST /api/trade2/search/poe2/{league}` from **inside the page** (in-page `fetch` — already Cloudflare-cleared and carries cookies):
   ...
   No browser open? Plain HTTP works just as well — but the User-Agent must be a **full, realistic browser UA string**; a bare `Mozilla/5.0` gets a Cloudflare 403 (verified 2026-07). If `.poe_cookies.json` exists in the project root, also send its cookies (see `../common/tricks.md` — cookie cache):
   ```bash
...
`poe2/knowledge/` accumulates POE2 experience-type facts that raw API data can't tell you — mod slot availability, market price wisdom, API/parsing quirks. It grows over many sessions, so **don't read it all**: grep `knowledge/INDEX.md` (and `../common/INDEX.md`) with keywords from the user's request — try both 中文 and English terms — and read only the file(s) that hit. **Never grep `../poe1/`.**
...
When the user teaches a new fact, decide the home first: **游戏知识 (mod/slot/機制/行情/欄位名/解析格式) → `poe2/knowledge/`；傳輸層或與遊戲無關的方法 (CF/限流/撈池法/交付格式) → `../common/`。** 拿不準就放 realm 目錄（放錯到 common 會污染 POE1，放錯到 poe2 只是重複一次）。Then verify any stat ids against `references/stats.tsv` and add a bilingual keyword line to the matching `INDEX.md`.
