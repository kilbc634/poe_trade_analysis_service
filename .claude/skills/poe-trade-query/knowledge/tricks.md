...
**Reusable implementation: [`scripts/gear_combo_optimizer.py`](../scripts/gear_combo_optimizer.py)** (self-contained stdlib script: 429-aware search/fetch helpers + mod parser with all known gotchas + the optimizer). Don't rewrite from scratch — copy it to the scratchpad, refresh `CURRENCY_RATES` from [exchange-rates.md](exchange-rates.md), edit the CONFIG/slots/constraints for the task. Extending to 5 slots (rings/belt) may need per-slot dominance pruning before the cross-product.
